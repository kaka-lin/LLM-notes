...
> [!NOTE]
> `local` 模式決定核心架構的部署方式。這與 IP 網路綁定無關，代表您要把「Gateway（負責對外通訊）」與「Agent（負責執行思考與操作的 AI）」整合在同一個實例或同個 Docker 容器內運作。這對於個人開發環境或輕量級伺服器而言是標準的配置方式。

## 3. 常見問題排除

> [!CAUTION]
> 當您遇到 `curl: (56) Recv failure: Connection reset by peer` 錯誤，通常代表 Gateway 目前處於 `loopback` 模式，擋掉了來自容器外部的連線請求。

### 解決步驟

1. 確認 `.env` 中有設定 `OPENCLAW_GATEWAY_BIND=lan`。
1. 確認 `docker-compose.yml` 有透過 `env_file: - .env` 載入設定。
1. 如果剛剛改完設定，務必要徹底重建容器：
...
1. 利用 `docker compose logs | grep "listening on"` 確認是否有正確綁定到 `0.0.0.0`。
